...
async function fetchNetflix(tmdbId, type, title, season, episode) {
    try {
        ...
        const audioTracks = (data.audioTracks || data.audios || data.audio || [])
            .filter((a) => (typeof a === 'object' && a.url) || typeof a === 'string')
            .map((a, i) => ({
...
async function fetchPlatform(platform, title, type, season, episode) {
    try {
        ...
        const audioTracks = (player.audioTracks || player.audios || player.audio || [])
            .filter((a) => (typeof a === 'object' && a.url) || typeof a === 'string')
            .map((a, i) => ({
                url: typeof a === 'string' ? a : a.url,
                lang: a.lang || a.language || a.label || a.name || 'Unknown',
                label: a.label || a.name || a.lang || 'Unknown',
                id: `netmirror-${platform}-audio-${i}`
            }));
...
module.exports = { getStreams: getNetmirrorStreams };