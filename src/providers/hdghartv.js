// src/providers/hdghartv.js
const { getDetails, resolveImdbId, tmdbTitleToImdbId } = require('../utils/tmdb');
...
function extractAudioTracks(linkData) {
    if (linkData.audioTracks && Array.isArray(linkData.audioTracks)) {
        return linkData.audioTracks.map((a) => ({
            url: a.url || a.file || '',
            lang: a.lang || a.language || a.label || 'Unknown',
            id: `hdghartv-audio-${a.lang || a.language || 'unknown'}`
        }));
    }
    if (linkData.tracks && Array.isArray(linkData.tracks)) {
        const audioTracks = linkData.tracks.filter(t => t.kind === 'audio' || t.type === 'audio');
        return audioTracks.map((a) => ({
            url: a.url || a.file || '',
            lang: a.lang || a.language || a.label || 'Unknown',
            id: `hdghartv-audio-${a.lang || a.language || 'unknown'}`
        }));
    }
    if (linkData.audios && Array.isArray(linkData.audios)) {
        return linkData.audios.map((a) => ({
            url: a.url || a.file || '',
            lang: a.lang || a.language || a.label || 'Unknown',
            id: `hdghartv-audio-${a.lang || a.language || 'unknown'}`
        }));
...
function linksToStreams(links, labelSuffix = '') {
    const sortedLinks = sortLinks(links.filter((l) => l.isActive && l.url));
    
    return sortedLinks.map((l) => {
        const audioTracks = extractAudioTracks(l);
...
module.exports = { getStreams: getHdghartvStreams };