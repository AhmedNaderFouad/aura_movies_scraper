...
async function getOnetouchtvStreams(tmdbId, mediaType = 'movie', seasonNum = null, episodeNum = null) {
    ...
    try {
        ...
        const audioTracks = [];
        const audioData = streamData.audio || streamData.audios || streamData.audioTracks || [];
        for (const a of audioData) {
            const audioUrl = typeof a === 'string' ? a : a.url || a.file;
            if (audioUrl) {
                audioTracks.push({
                    url: audioUrl,
                    lang: a.lang || a.language || a.name || a.label || 'Unknown',
...
module.exports = { getStreams: getOnetouchtvStreams };