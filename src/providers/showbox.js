...
const getShowboxUrlFromTmdbInfo = async (tmdbType, tmdbId, regionPreference = null) => {
    ...
    const findUrl = async () => {
                ...
                if (htmlContent) {
                    ...
                    if (pageInfo && pageInfo.title) {
                        ...
                        if (titleIsValid && imageIsValid) {
                            ...
                            return { showboxUrl: directShowboxUrl, year: year, title: mainTitle, originalLanguage: tmdbData.original_language };
                        }
                    }
                }
            }
        ...
        if (searchResult && searchResult.url) {
            ...
            return { showboxUrl: searchResult.url, year: year, title: mainTitle, originalLanguage: tmdbData.original_language };
        }
...
const generateLanguageStreams = async (masterM3u8Url, baseStream, originalLanguage = 'en') => {
    try {
        const response = await axios.get(masterM3u8Url, {
            headers: {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('#EXT-X-MEDIA:TYPE=AUDIO')) {
                ...
                if (uriMatch) {
                    ...
                    const uri = uriMatch[1].startsWith('http') ? uriMatch[1] : baseUrl + uriMatch[1];
                    const language = languageMatch ? languageMatch[1] : 'Unknown';

                    audioTracks.push({
                        language: language,
                        languageName: nameMatch ? nameMatch[1] : 'Unknown',
                        uri: uri,
                        isDefault: language.toLowerCase() === originalLanguage.toLowerCase()
                    });
                }
            }
        }

        // Consolidation: If no matching original language found, set first as default or English
        if (audioTracks.length > 0 && !audioTracks.some(t => t.isDefault)) {
            const engTrack = audioTracks.find(t => t.language.toLowerCase() === 'en');
            if (engTrack) engTrack.isDefault = true;
            else audioTracks[0].isDefault = true;
        }

        // Sort audio tracks: Default (Original) first
        audioTracks.sort((a, b) => (b.isDefault ? 1 : 0) - (a.isDefault ? 1 : 0));

        // Grouping: Instead of splitting into multiple streams, we return ONE stream with ALL audio tracks.
        const consolidatedStream = { ...baseStream };
        consolidatedStream.url = masterM3u8Url;
        consolidatedStream.audioTracks = audioTracks;

        // Pick primary language for backward compatibility fields
        const defaultTrack = audioTracks.find(t => t.isDefault) || audioTracks[0];
        if (defaultTrack) {
            consolidatedStream.language = defaultTrack.language;
            consolidatedStream.languageName = defaultTrack.languageName;
            consolidatedStream.audioUri = defaultTrack.uri;
            consolidatedStream.isDefault = true;
        }

        return [consolidatedStream];
    } catch (error) {
        console.log(`[ShowBox] Error generating language streams: ${error.message}`);
        return [{ ...baseStream, url: masterM3u8Url, language: 'Original', isDefault: true, audioTracks: [] }];
    }
};


const getStreams = async (tmdbId, mediaType = 'movie', seasonNum = null, episodeNum = null, regionPreference = null, userCookie = null) => {
    console.log(`[ShowBox] getStreams called: tmdbId=${tmdbId}, type=${mediaType}, season=${seasonNum}, episode=${episodeNum}`);
    
    try {
        const tmdbType = mediaType === 'tv' ? 'tv' : 'movie';
        const tmdbInfo = await getShowboxUrlFromTmdbInfo(tmdbType, tmdbId, regionPreference);
        const originalLanguage = tmdbInfo?.originalLanguage || 'en';
        ...
        const allLanguageStreams = [];
        for (const stream of streams) {
            if (stream.url && stream.url.includes('.m3u8')) {
                const languageStreams = await generateLanguageStreams(stream.url, stream, originalLanguage);
                allLanguageStreams.push(...languageStreams);
            } else {
                allLanguageStreams.push(stream);
            }
        }
        
        return allLanguageStreams;
    } catch (error) {
...
const scrape = async (tmdbId, mediaType = 'movie', seasonNum = null, episodeNum = null) => {
    console.log(`[ShowBox] scrape called: tmdbId=${tmdbId}, type=${mediaType}`);
    return await getStreams(tmdbId, mediaType, seasonNum, episodeNum);
};
