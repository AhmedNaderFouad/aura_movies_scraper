...
const videasyClient = axios.create({
...
async function getVideasyStreams(tmdbId, mediaType = 'movie', seasonNum = null, episodeNum = null, region = null, cookie = null, baseUrl = '') {
    ...
    let seed;
    try {
        const seedRes = await videasyClient.get(`${VIDEASY_API}/seed?mediaId=${tmdbId}`, { timeout: 8000 });
    ...
    const queryServers = async (activeSeed) => {
        await Promise.all(Object.entries(SERVERS).map(async ([name, server]) => {
            ...
            try {
                const encRes = await videasyClient.get(apiUrl, {
                    timeout: 8000,
                    responseType: 'text'
                });
                ...
                const audioTracks = (resData.audioTracks || resData.audios || resData.audio || [])
                ...
                for (const s of resData.sources) {
                    ...
                    const finalStreamUrl = baseUrl 
                        ? `${baseUrl}/api/proxy-stream?url=${encodeURIComponent(s.url)}` 
                        : s.url;
    ...
    // =============================================
    // 🔥 السيناريو المثالي: ترتيب وتصفية الروابط
    // =============================================
    const sortedStreams = rawStreams.sort((a, b) => {
        const aIsVimeos = a.rawUrl.includes('vimeos.net') || a.serverName === 'LaMovie';
        const bIsVimeos = b.rawUrl.includes('vimeos.net') || b.serverName === 'LaMovie';

        // 1. أولوية مطلقة لسيرفر LaMovie (Vimeos) في أول القائمة
        if (aIsVimeos && !bIsVimeos) return -1;
        if (!aIsVimeos && bIsVimeos) return 1;

        // 2. ترتيب بافي الجودات (1080p ثم 720p ثم 480p)
    ...
    // تنظيف المخرجات من الخواص المؤقتة
    const finalStreams = sortedStreams.map(({ serverName, rawUrl, ...stream }) => stream);

    console.log(`[Videasy] Total processed streams: ${finalStreams.length}`);
    return finalStreams;
}
